...
const App = () => {
  ...
  const counterSlice = Math.floor(count / 5);
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-4 h-screen">
      ...
      <div className="flex flex-wrap gap-4 items-center bg-slate-50 p-10 border border-purple-200 rounded-xl">
        {Array.from({ length: counterSlice }).map((item, idx) => (
          <div key={idx} className="bg-secondary p-2 px-2 rounded-md">
            {idx < 9 ? `0${idx + 1}` : idx + 1}
          </div>
        ))}
      </div>
